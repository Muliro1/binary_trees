...
/**
 * get_position - Gets the next available insertion position \
 ...
 */
heap_t *get_position(const heap_t *root)
{
	...
	if (root != NULL)
	{
		queue_item_3(&head, &tail, &n, (void *)root);
		while (n > 0)
		{
			current = head;
			if (current->parent != NULL)
			{
				...
				{
					queue_item_3(&head, &tail, &n, (void *)(current->parent->left));
					if (current->parent->right != NULL)
					{
						queue_item_3(&head, &tail, &n, (void *)(current->parent->right));
					}
					else
					{
 ...
 */
void swap_with_parent(heap_t **node, heap_t **root)
{
		...
		else
		{
			...
			while (new_node->parent != NULL)
			{
				if (new_node->n > new_node->parent->n)
					swap_with_parent(&new_node, root);
				else
					break;
			}
		}
